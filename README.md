...
### [→ Live Demo](https://pitlane-live-f1.vercel.app)

**Demo video:** [Watch](VIDEO_URL_HERE)

[![Live App](https://img.shields.io/badge/Live_App-pitlane--live--f1-brightgreen?style=for-the-badge&logo=vercel)](https://pitlane-live-f1.vercel.app)
...
This is not a tutorial project. It is a production deployment with a real user-facing frontend, a backend that handles concurrent requests under load, an async post-race data pipeline, and two infrastructure migrations behind it.

**Live:** https://pitlane-live-f1.vercel.app
...
**Fix:** switch to gthread workers. gthread uses real OS threads, so there is no monkey patching and no SSL conflict, with the same concurrency benefit within the 512MB memory constraint.
...
FastF1 with pandas needs 1 to 2GB RAM during race data processing. Render's free tier has 512MB. Attempting on-demand processing would OOM the server.

**Fix:** process locally after each race, commit the output JSONs to the repo, let Render redeploy with new data. The 24MB total JSON payload is well within GitHub's file limits. The tradeoff is a 2 to 3 minute update delay after each race, acceptable for a post-race analytics app.

### 3. Atomic JSON write pattern

The race replay endpoint reads from a pre-built JSON file on disk. If a client requests `/api/replay/2026/5` exactly as that file is being written by the pipeline, they would read a partial file and potentially invalid JSON.

**Fix:** write to a `.tmp` file first, then do an atomic `os.replace()` to move it into the target path. `os.replace()` is atomic at the OS level, so the old file is never partially visible to readers.
...
Render's free tier sleeps the service after 15 minutes of inactivity, and a cold start adds roughly 30 seconds to the first request.

**Fix:** a cron-job.org job pings `/api/schedule` every 10 minutes, keeping the service warm at zero cost.
...
Deployed on a Microsoft Azure B2s VM (Ubuntu 22.04, 2 vCPUs, 4GB RAM) via an Azure for Students subscription. Flask served via Gunicorn and gevent workers behind Nginx. SQLite on the VM filesystem. FastF1 processing ran directly on the VM.
...
Migration trigger: Azure student credits exhausted. The VM was also over-provisioned for this workload, running 4GB RAM continuously for an API that serves bursts of traffic around race weekends.
...
Run within 30 to 60 minutes of the chequered flag.
...
Render auto-redeploys in 2 to 3 minutes.

**Step 3 — Trigger scoring:**
```bash
curl -X POST https://pitlane-live-f1.vercel.app/api/scoring/score-race \
...
|---|---|---|
| GET | `/api/schedule` | Full 2026 race calendar and next session countdown |
| GET | `/api/news` | Latest F1 headlines (Autosport RSS, 10-minute cache) |
| GET | `/api/replay/:year/:round` | Lap-by-lap position data for race replay |
| POST | `/api/scoring/score-race` | Admin, triggers post-race scoring (requires `X-Admin-Key`) |
...
Environment variables: `DATABASE_URL`, `SECRET_KEY`, `ADMIN_KEY`, set in the Render dashboard.
