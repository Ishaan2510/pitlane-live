      ...
      <div class="hero-inner">
        ...
        <div class="hero-main">
          ...
          <div class="countdown" v-if="countdown.total > 0">
            <div class="cd-unit" v-for="unit in countdownUnits" :key="unit.label">
              <div class="cd-digits">
                <span
        ...
        <div class="hero-actions">
          ...
          <router-link to="/leaderboard" class="hero-btn ghost">
            Standings
          </router-link>
        </div>
      ...
      <div class="ticker" v-if="upcomingRaces.length > 1">
        ...
        <div class="ticker-items">
          <span
            ...
            class="ticker-item"
          >
            RD {{ race.round }} &mdash; {{ race.name }} &mdash; {{ formatDate(race.date) }}
          </span>
        </div>
      </div>
    </section>
...
export default {
  ...
  computed: {
    ...
    countdownUnits() {
      return [
        {
          label:    'DAYS',
          digits:   String(this.countdown.days).padStart(2, '0').split(''),
          flipping: this.flipStates.days
        },
        {
          label:    'HRS',
          digits:   String(this.countdown.hours).padStart(2, '0').split(''),
          flipping: this.flipStates.hours
        },
        {
          label:    'MIN',
          digits:   String(this.countdown.minutes).padStart(2, '0').split(''),
          flipping: this.flipStates.minutes
        },
        {
          label:    'SEC',
          digits:   String(this.countdown.seconds).padStart(2, '0').split(''),
          flipping: this.flipStates.seconds
        },
      ]
    }
  },
  ...
  methods: {
    ...
    tickCountdown() {
      ...
      const raceDate = new Date(this.nextRace.date + 'T14:00:00Z') // approximate race start
      const now      = new Date()
    ...
    navigateToRace(race) {
      ...
      // upcoming — do nothing (no action yet)
    },
...
.cd-digit.flip {
  animation: flip-digit 0.35s ease;
}
...
.countdown > .cd-unit:not(:last-child)::after {
  content: ':';
  position: absolute;
  font-family: 'Bebas Neue', sans-serif;
  font-size: 2rem;
...
/* Next race — subtle top accent line */
.cal-row.next {
...
.cal-row.next   .cr-action .next-cta   { opacity: 1; }
